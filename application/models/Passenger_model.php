<?php
    ...
    class Passenger_model extends CI_Model{
        ...
        public function c_tiketClient($where){
            $this->db->select('*');
            $this->db->from('booking');
            $this->db->join('detail_booking','booking.id_booking=detail_booking.id_booking');
            $this->db->join('tarif_penerbangan ','detail_booking.id_tarif=tarif_penerbangan.id_tarif');
        ...
        public function cetak_invoice_bukti($where){
            return $this->db->query("SELECT customer.*,detail_booking.id_detail, penerbangan.*,tarif_penerbangan.tarif_bisnis,tarif_penerbangan.tarif_ekonomi,pesawat.*,booking.*, bandara.* FROM penerbangan INNER JOIN booking ON booking.id_penerbangan=penerbangan.id_penerbangan INNER JOIN detail_booking ON detail_booking.id_booking = booking.id_booking INNER JOIN tarif_penerbangan ON tarif_penerbangan.id_penerbangan=penerbangan.id_penerbangan INNER JOIN customer ON booking.id_customer=customer.id_customer INNER JOIN bandara ON bandara.id_bandara=penerbangan.id_bandara INNER JOIN pesawat ON pesawat.id_pesawat=penerbangan.id_pesawat WHERE booking.id_customer = '$where' AND booking.status_bayar='BOOKING'");
        }
